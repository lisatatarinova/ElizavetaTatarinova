...
public class ListRequests extends MainRequests {
    public ListDto create(String listName, String boardId) {
        ListDto requestBody = new ListDto();
        requestBody.setName(listName);
        requestBody.setIdBoard(boardId);
        ListDto response = super.create(requestBody, LIST_URI).as(ListDto.class);
        return response;
    }

    public ListDto updateListData(String newValue, ListDto list) {
        super.updateData(newValue, MessageFormat.format("{0}{1}", LIST_URI, list.getId()))
                .as(ListDto.class);
